...
struct AnthropicRewriteService: TextRewriteService {
    ...
    private func withRetry<T>(
        ...
        for attempt in 1...attempts {
            do {
                ...
                // 仅对 429 (Rate Limit) 和 5xx (Server Error) 进行重试
                if error.shouldRetry && attempt < attempts {
                    let delay = pow(2.0, Double(attempt))  // 指数退避: 2s, 4s...
                ...
                if attempt < attempts {
                    try await Task.sleep(nanoseconds: UInt64(1_000_000_000))  // 网络错误简单重试
                    continue
                }
                throw error
            }
