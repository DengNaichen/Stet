...
actor OpenAIClient {
    ...
    private final class MultipartUploadProgressDelegate: NSObject, URLSessionTaskDelegate, @unchecked Sendable {
        private let lock = NSLock()
        private var didMarkUploadCompletion = false
        ...
        func urlSession(
            ...
            guard shouldMarkUploadCompletion(totalBytesSent: totalBytesSent, totalBytesExpectedToSend: totalBytesExpectedToSend) else {
                return
            }

            Task {
                await DictationLatencyProbe.shared.ensureUploadCompleted()
            }
        }

        func markUploadCompletedIfNeeded(note: String? = nil) async {
            guard shouldMarkUploadCompletion(totalBytesSent: 1, totalBytesExpectedToSend: 1) else { return }
            await DictationLatencyProbe.shared.ensureUploadCompleted(note: note)
        }

        private func shouldMarkUploadCompletion(
            totalBytesSent: Int64,
            totalBytesExpectedToSend: Int64
        ) -> Bool {
            lock.lock()
            ...
            guard !didMarkUploadCompletion,
                  totalBytesSent >= totalBytesExpectedToSend else {
                return false
            }

            didMarkUploadCompletion = true
            return true
        }
    }
    ...
    func sendMultipartRequest<Response: Decodable>(
        ...
        return try await execute(
            ...
            trackLatencyProbe: trackLatencyProbe
        )
    }
    ...
    private func execute<Response: Decodable>(
        ...
        if trackLatencyProbe {
            let progressDelegate = MultipartUploadProgressDelegate()
            dataAndResponse = try await session.data(for: request, delegate: progressDelegate)
            await progressDelegate.markUploadCompletedIfNeeded(note: "response_fallback")
        } else {
