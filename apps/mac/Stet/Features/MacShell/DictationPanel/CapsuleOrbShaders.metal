...
namespace Config {
    ...
    constant float CLOUD_WHITE_BASE = 0.90;
    constant float CLOUD_WHITE_AMP = 0.10;
    ...
    constant float SUN_STRENGTH = 0.10;
...
[[ stitchable ]] half4 cloudOrbGlassWide(
    ...
    pRaw.x += Config::MOTION_X_AMP * sin(time * Config::MOTION_X_FREQ + pRaw.y * Config::MOTION_X_Y_SKEW);
    ...
    float centerPush = (Config::SHAPE_PUSH_BASE + Config::SHAPE_PUSH_SKEW * breath) * coreY * exp(-abs(pRaw.x) * 1.20);
    ...
    float fluidBias = p.y * Config::FLUID_Y_BIAS + (w.y - 0.5) * Config::FLUID_WARP_BIAS;
    float colorLift = centerMask * breath * Config::COLOR_LIFT_FACTOR;

    float maskBlue = smoothstep(Config::MASK_BLUE_LOW, Config::MASK_BLUE_HIGH, n - fluidBias * Config::MASK_BLUE_SKEW - colorLift);
    float maskOrange = smoothstep(Config::MASK_ORANGE_LOW, Config::MASK_ORANGE_HIGH, w.x + fluidBias * Config::MASK_ORANGE_SKEW + colorLift);
    ...
    float cloudDense = smoothstep(Config::CLOUD_DENSE_LOW, Config::CLOUD_DENSE_HIGH, n + centerMask * breath * Config::COLOR_LIFT_FACTOR);
    float cloudSoft = smoothstep(Config::CLOUD_SOFT_LOW, Config::CLOUD_SOFT_HIGH, Config::CLOUD_SOFT_n_SKEW * n + Config::CLOUD_SOFT_w_SKEW * w.x);
    float cloudLift = smoothstep(Config::CLOUD_LIFT_LOW, Config::CLOUD_LIFT_HIGH, Config::CLOUD_LIFT_n_SKEW * n + Config::CLOUD_LIFT_w_SKEW * w.y + centerMask * breath * Config::CLOUD_LIFT_CENTER_SKEW);
    ...
    base = mix(base, float3(1.0), cloud * (Config::CLOUD_WHITE_BASE + Config::CLOUD_WHITE_AMP * breath));

    float breathGlow = exp(-pRaw.y * pRaw.y * Config::GLOW_Y_SKEW) * exp(-pRaw.x * pRaw.x * Config::GLOW_X_SKEW);
    base += Config::GLOW_COLOR_BASE * breathGlow * breath * Config::GLOW_INTENSITY;
