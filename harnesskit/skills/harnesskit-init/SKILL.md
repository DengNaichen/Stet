...
把预告作为面向用户的阶段过渡，而不是运行日志、审计日志或工具状态。只在以下有意义边界先预告再继续：进入预热轮、进入每个 artifact 固定循环、发射每次确认轮（含同一 artifact 的每个溢出轮）、continuation/resume，以及开始最终验证；预告本身无需回复，不要等待用户回复，也不要为这些边界之间的内部小步骤制造 progress spam。

每条预告默认写成 1 个短段落、2–3 个短句。先用用户可理解的语言说明本阶段要解决什么问题，再把 2–4 类 repo-specific 真实来源以自然名称融入说明，解释为什么查看它们以及过滤后预计出现的问题类型和大致规模，最后明确用户现在是否需要操作、下一次可能在哪个决定点参与；来源类别必须来自当时可用的 kickoff/artifact 地图与对应 owner 的最低扫描面，不得机械逐字段罗列、使用与实际扫描无关的固定模板空话或暴露未过滤候选。

默认不要提及对用户判断无帮助的内部机制、命令或统计，包括 `sidecar`、`receipt`、`run-state`、`final verifier`、`intent discovery`、allocated ID、内部脚本路径与门禁计数。只有它们直接影响用户决定或造成阻塞时，才用普通语言解释其影响。

以下内容只示范表达方式，不得照抄为固定模板，也不得据此固定 artifact 或扫描面：

- Artifact 进入：“接下来我会整理项目的架构边界，主要对照源码目录、构建配置和依赖声明，确认模块职责与依赖方向是否一致。现在不用操作；如果发现仓库本身无法回答的长期约束，我会在扫描完成后请你确认。”
